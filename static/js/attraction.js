...
window.onload = async function () {
    ...
    document.getElementById("loadingScreenA").style.display = "none";//網頁最後都會跑這個function，所以可以把loadind畫面加這
...
select_travel_date()
function select_travel_date(){
  let today = new Date()
  let year = today.getFullYear()
  let month = today.getMonth()+1 < 10 ? "0"+(today.getMonth()+1) : today.getMonth()+1 //月份兩位數的形式顯示
  let date = today.getDate() <10 ? "0"+today.getDate() : today.getDate()//日期兩位數的形式顯示
  let min_date = `${year}-${month}-${date}`//格式為 yyyy-mm-dd
  document.getElementsByClassName("attraction_tour_booking_trip_calendar")[0].min = min_date//min 屬性通常用於 <input> 元素的日期輸入，表示可選擇的最小日期
}
