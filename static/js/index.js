...
let targetElement = document.getElementById('observer_footer'); // 替換為你的觸發元素
intersectionObserver.observe(targetElement);
let loadedElements = []; // 創建全域陣列來儲存已載入的元素
// 載入下一頁的資料
function loadNextPage() {
    ...
    fetch(`/api/attractions?page=${nextPage}`)
        ...
        .then(data => {
            ...
            if (dataLength > 0) {
                let imagesLoaded = 0; // 用來計數載入完成的圖片數量
                // 處理從API返回的資料
                data.data.forEach(attraction => {
                    ...
                    // 創建圖片<img>元素
                    let image = new Image();//let image = document.createElement('img');
                    image.src = attraction.images[0];
                    image.alt = 'picture';

                    let observer_loadingScreen = document.getElementById('observer_loadingScreen');
                    loadedElements.push(attractionItem); // 添加到已載入的元素陣列中


                    // 圖片載入完成時
                    if (observer_loadingScreen) {
...
function closeLoadingScreen() {
    ...
    let observer_loadingScreen = document.getElementById("observer_loadingScreen");
    observer_loadingScreen.style.display = "none";
...
function showLoadingScreen() {
    let observer_loadingScreen = document.getElementById("observer_loadingScreen");
    observer_loadingScreen.style.display = "block";
}
